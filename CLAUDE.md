...
The builder emits a **second file from the same panel list**: `grafana/dashboard.grafana-com.json`, the copy uploaded to the grafana.com community catalog. It differs only in datasource wiring — grafana.com's uploader rejects any dashboard without an `__inputs` block as "Old dashboard JSON format" (regardless of `schemaVersion`), so the catalog variant carries `__inputs`/`__requires`, drops the pinned `DS_PROMETHEUS` template var, and drops `uid` so importers mint their own. Never hand-edit it or round-trip it through Grafana's UI export — rerun the builder, which keeps both files in sync by construction. `GRAFANA_MIN_VERSION` in the builder is the version advertised in `__requires`; bump it if panel options requiring a newer Grafana get added.
