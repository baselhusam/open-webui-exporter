...
data.model_id, so positive/negative tallies are computed per model here.

(The sibling /feedbacks/models endpoint only returns a flat list of model-id
strings with no counts, and /leaderboard returns pre-aggregated Elo entries
already sorted best-first.)
"""

from collectors.client import get_json
from metrics import MODEL_FEEDBACK_NEGATIVE_TOTAL, MODEL_FEEDBACK_POSITIVE_TOTAL, MODEL_LEADERBOARD_RANK
...
def collect_feedback(session, base_url):
    ...
    for model_id in set(positive) | set(negative):
        MODEL_FEEDBACK_POSITIVE_TOTAL.labels(model=model_id).set(positive.get(model_id, 0))
        MODEL_FEEDBACK_NEGATIVE_TOTAL.labels(model=model_id).set(negative.get(model_id, 0))
